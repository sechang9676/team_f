...
public class Store {
	...
	public static int[] buttonID = { Value.airCellA, Value.airCellB, Value.airAir, Value.airAir, Value.airAir,
			Value.airAir, Value.airAir, Value.airTrash };
	public static int[] buttonPrice = { 1, 1, 0, 0, 0, 0, 0, 0 };
	...
	public void define() {
		for (int i = 0; i < button.length; i++) {
			button[i] = new Rectangle(
					...
					(Screen.room.block[Screen.room.worldHeight - 1][0].y) + (Screen.room.blockSize + shopSpacing),
	...
	public void draw(Graphics g) {
		...
		for (int i = 0; i < button.length; i++) {
			...
			if (buttonPrice[i] > 0) {
				...
				g.drawString("$" + buttonPrice[i] + "", button[i].x + itemIn + 10,
						button[i].y + itemIn + button[i].height - 12);
			}

		}
		g.drawImage(Screen.tileset_res[2], buttonHealth.x, buttonHealth.y, buttonHealth.width, buttonHealth.height,
				null);
		...
		g.drawString("" + Screen.health, buttonHealth.x + buttonHealth.width + iconSpace,
				buttonHealth.y + buttonHealth.height + iconSpace - iconTextY);// Draw Health
		g.drawString("" + Screen.coinage, buttonCoins.x + buttonCoins.width + iconSpace,
				buttonCoins.y + buttonCoins.height + iconSpace - iconTextY);// Draw Coins

		g.setFont(new Font("Arial", Font.BOLD, 14));

		g.setColor(new Color(255, 255, 255));
		g.drawString("Level: " + Screen.level, button[button.length - 1].x + 100,
				buttonHealth.y + buttonHealth.height + iconSpace - iconTextY);// Draw Level

		if (holdsItem) {
			g.drawImage(Screen.tileset_air[heldID], Screen.mse.x - ((button[0].width - (itemIn * 2)) / 2) + itemIn,
					Screen.mse.y - ((button[0].width - (itemIn * 2)) / 2) + itemIn, button[heldID].width - (itemIn * 2),
					button[heldID].height - (itemIn * 2), null);
		}
	}
}
